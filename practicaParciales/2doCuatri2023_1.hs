...
problema formulasInvalidas (formulas: seq⟨String x String⟩) : Bool {
 requiere: {True}
 asegura: {(res = true) <=> formulas no contiene nombres repetidos, es decir que cada candiadato esta en mas de una formula (no puede ser candidato a presidente y a vicepresidente ni en la misma formula ni en formulas distintas)  }
------------------------------------------
