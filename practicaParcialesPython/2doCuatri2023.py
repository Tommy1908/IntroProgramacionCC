...
#print(acomodar(["UP","LLA","LLA","LLA","LLA","LLA","LLA","UP","UP","LLA","UP","UP","UP","LLA","LLA","UP","UP","LLA","LLA","LLA","UP","UP","UP","UP","UP","LLA","UP","UP","UP"]))
...
#print(pos_umbral([1,4,-1,-8,0,1,0,-5,0,6,8,0,-1],999))
...
def columnas_repetidas(mat: list[list[int]]) -> bool:
    res: bool = True
    #Por requiere la longitud de cada elemento de m es la misma
    longitudMitad: int = int(len(mat[0])/2)
    for i in range(len(mat)):
        for k in range(longitudMitad):
            index1:int = 0 + k
            index2:int = longitudMitad + k
            if mat[i][index1] != mat[i][index2]:
                res = False
    return res
...
#print(cuenta_posiciones_por_nacion(["nz", "sud", "aus", "arg", "chl", "nqn"],{2023:["arg", "sud", "nz", "aus", "chl", "nqn"], 2022:[ "chl", "nqn","nz", "sud", "aus", "arg"],2024:["nqn","arg","chl","nz","sud","aus"],4456:["nqn","arg","chl","nz","sud","aus"]}))